(function (e, t) {
    ...
    x.fn.extend({
      ...
      addClass: function (e) {
        ...
        ]; s > a; a++) if (n = this[a], r = 1 === n.nodeType && (n.className ? (' ' + n.className + ' ').replace(U, ' ') : ' ')) {
          o = 0;
    ...
    x.extend({
      ...
      ajax: function (e, n) {
        ...
        if (h.promise(C).complete = g.add, C.success = C.done, C.error = C.fail, p.url = ((e || p.url || yn) + '').replace(xn, '').replace(kn, mn[1] + '//'), p.type = n.method || n.type || p.method || p.type, p.dataTypes = x.trim(p.dataType || '*').toLowerCase().match(T) || [
          ''
        ], null == p.crossDomain && (r = En.exec(p.url.toLowerCase()), p.crossDomain = !(!r || r[1] === mn[1] && r[2] === mn[2] && (r[3] || ('http:' === r[1] ? '80' : '443')) === (mn[3] || ('http:' === mn[1] ? '80' : '443')))), p.data && p.processData && 'string' != typeof p.data && (p.data = x.param(p.data, p.traditional)), qn(An, p, n, C), 2 === b) return C;
        l = p.global,
  ...
  (function (t, e, n) {
    ...
    function i(t, e, n, r, a, o) {
      ...
      if (1 === d.length && (c = d[0], d = [
      ], d[0] = c, d[1] = c), - 1 !== ('' + d[0]).indexOf('%')) s = parseFloat(d[0]) / 100,
      l = n.width * s,
      ...
      function t(t) {
        return function (e) {
          try {
            ...
            n.log(['failed to add color stop: ',
            r,
            '; tried to add: ',
            e])
          }
    ...
    u.Parse = function (r, a) {
      ...
      function v(t, e, n) {
        ...
        ue.trimText(e.nodeValue).length > 0 && (e.nodeValue = f(e.nodeValue, ge(t, 'textTransform')), c = c.replace(['-webkit-auto'], [
          'auto'
        ]), o = !a.letterRendering && /^(left|right|justify|auto)$/.test(c) && p(ge(t, 'letterSpacing')) ? e.nodeValue.split(/(\b| )/) : e.nodeValue.split(''), r = m(i, t, s, l), a.chinese && o.forEach(function (t, e) {
      ...
      function A(t, e, n, r, a, o, i) {
        ...
        r[0].start.x,
        r[0].start.y]), r[0].curveTo(t), r[1].curveTo(t)) : t.push(['line',
        o,
        i]),
      ...
      function B(t, e, n, r, a, o, i) {
        ...
        return e[0] > 0 || e[1] > 0 ? (l.push(['line',
        r[1].start.x,
        r[1].start.y]), r[1].curveTo(l)) : l.push(['line',
        t.c1[0],
        t.c1[1]]),
        n[0] > 0 || n[1] > 0 ? (l.push(['line',
        o[0].start.x,
        o[0].start.y]), o[0].curveTo(l), l.push(['line',
        i[0].end.x,
        i[0].end.y]), i[0].curveToReversed(l)) : (l.push(['line',
        t.c2[0],
        t.c2[1]]), l.push(['line',
        t.c3[0],
        t.c3[1]])),
        e[0] > 0 || e[1] > 0 ? (l.push(['line',
        a[1].end.x,
        a[1].end.y]), a[1].curveToReversed(l)) : l.push(['line',
        t.c4[0],
        t.c4[1]]),
        l
      }
        ...
        if (a && a.content && 'none' !== a.content && '-moz-alt-content' !== a.content && 'none' !== a.display) {
          ...
          Object.keys(a).filter(W).forEach(function (t) {
            try {
              ...
              ue.log(['Tried to assign readonly property ',
              t,
              'Error:',
              e])
            }
  ...
  !function (c) {
    ...
    var b = {
      ...
      parseFormat: function (j, h) {
        var g = j.replace(this.validParts(h), '\x00').split('\x00'),
    ...
    var methods = {
      destroy: function () {
        this.each(function (i, el) {
          ...
          if (helpers[guid + '-isMobile']) {
            mEvt.down = 'touchstart';
            mEvt.up = 'touchend';
            mEvt.move = 'touchmove'
          } else $(document).off('keydown.' + guid).off('keyup.' + guid);
      ...
      startAt: function (pct, animated) {
        this.each(function (i, el) {
          ...
          if (helpers[guid + '-colorChange']) follow[0].children[0].style.opacity = pct / 100
        });
        return this
      },
      init: function (options) {
        this.each(function (i, el) {
          ...
          if (uAgent.match(/Android/i) || uAgent.match(/webOS/i) || uAgent.match(/iPhone/i) || uAgent.match(/iPad/i) || uAgent.match(/iPod/i) || uAgent.match(/BlackBerry/i)) {
            helpers[guid + '-isMobile'] = true;
            mEvt.down = 'touchstart';
            mEvt.up = 'touchend';
            mEvt.move = 'touchmove';
            if (window.navigator.msPointerEnabled) {
              mEvt.down = 'MSPointerDown';
              mEvt.up = 'MSPointerUp';
              mEvt.move = 'MSPointerMove'
            }
            var touchX = null,
            touchY = null
          ...
          if (imageBln) {
            ...
            self.css({
              'border-radius': (r_corners ? d + 'px' : '0'),
              'overflow': 'hidden'
            });
          ...
          follow.css({
            'position': (!settings.disabled ? 'absolute' : 'static'),
          ...
          var drawSnapmarks = function (resize) {
            ...
            if (markers) {
              ...
              var str = '';
              for (var i = 0; i < snapValues.length; i++) str += '<div style="display:inline-block; width:0; height:5px; border-left:#333 solid 1px; position:relative; left:' + (snapValues[i] - i) + 'px; float:left"></div>';
              marks.html(str)
            }
          };
          var verticalTransform = function () {
            if (markers && snaps > 0 && snaps < 10) {
              ...
              a.wrapAll('<div id="' + guid + '_vert-marks" style="margin:0; z-index:997; width:' + width + unit + '; -webkit-backface-visibility:hidden; -moz-backface-visibility:hidden; -ms-backface-visibility:hidden; backface-visibility:hidden"></div>');
              var vmarks = $('#' + guid + '_vert-marks');
              self.css('width', '100%');
              vmarks.css(cssContentBox).css(cssRotate);
              for (var i = 0; i < a.length; i++) a.css('margin', '0')
            } else {
              self.css({
                'margin': '0',
                'top': '0',
          ...
          btnTriggers = function (direction, smoothBln) {
            ...
            set_value = THE_VALUE;
            if ((THE_VALUE + knob_adjust) > 100) {
              THE_VALUE = 100;
              set_value = 100
            } else if (THE_VALUE - knob_adjust < 0) {
              THE_VALUE = 0;
              set_value = 0
            }
          ...
          $(document).on(mEvt.move + '.' + guid, function (e) {
            if (is_down) {
              ...
              result = knob[0].style.left;
              result = result.replace('px', '');
              var state = self.data('state');
              if (options.drag && state == 'active') options.drag(updateME(getPercent(result)));
              if (colorChangeBln && state == 'active') colorChange(getPercent(result))
            }
            ...
            if (state == 'active') {
              ...
              stopper = knobWidth / 2,
              m = x - stopper;
              if (snaps > 0 && snaps < 10 && (snapType == 'soft' || snapType == 'hard')) result = doSnap((snapType == 'hard') ? 'hard' : 'soft', m);
               else result = (m < 0 ? 0 : m);
